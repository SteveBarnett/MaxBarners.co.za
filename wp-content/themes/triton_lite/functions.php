...
function trt_widgets_init(){
	...
	register_sidebar(array(
	'name'          => __('Midrow Widgets'),
	'id'            => 'mid_sidebar',
	'description'   => 'Widget Area for the Midrow',
...
function trt_credits_meta( $post ) {
  ...
  wp_nonce_field( plugin_basename( __FILE__ ), 'trt_noncename' );

  $trtdata = get_post_meta($post->ID, 'trt_slide_link', TRUE);
  wp_nonce_field( 'trt_meta_box_nonce', 'meta_box_nonce' ); 

  // The actual fields for data entry
  echo '<input type="text" id="trt_sldurl" name="trt_sldurl" value="'.$trtdata['trt_sldurl'].'" size="75" />';
}
...
if( !isset( $_POST['meta_box_nonce'] ) || !wp_verify_nonce( $_POST['meta_box_nonce'], 'trt_meta_box_nonce' ) ) return; 
  ...
  foreach($_POST as $key => $data) {
    if($key == 'trt_meta_box_nonce')
      continue;
    if(preg_match('/^trt/i', $key)) {
      $trtdata[$key] = $data;
    }
  }
