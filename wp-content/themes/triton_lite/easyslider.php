...
<div id="slider" class="easyslider">
        ...
        <?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
         ...
         <?php $trtdata = get_post_meta($post->ID, 'trt_slide_link', TRUE); ?>
            <?php the_title( '<h2 class="entry-title"><a href="' . $trtdata['trt_sldurl'] . '" title="' . the_title_attribute( 'echo=0' ) . '" rel="bookmark">', '</a></h2>' ); ?>
                <a href="<?php echo $trtdata['trt_sldurl']; ?>"><?php the_excerpt(); ?></a>
            </div>  
